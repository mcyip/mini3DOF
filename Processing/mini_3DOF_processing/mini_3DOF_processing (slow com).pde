...
final String serialPort = "COM6";  // change this to your port on linux. Should be something like "/dev/ttyACM0"
...
void setup() 
{
  ...
  myPort.write(0); //send initial force x to start the device
  myPort.write(0); //send initial force y to start the device
  myPort.write(0); //send initial force z to start the device
...
//this will convert a String to a set of values. The String is specially formatted so that every n chars is one full integer.
int convertsCharStringToValue(String str, int firstChar, int numOfChars){
  int value = 0;
  for(int i = 0; i < numOfChars; i++){
      char val = str.charAt(firstChar+(numOfChars-i-1));
      value = value << 8;
      value = value | val;
  }
  return value;
...
int readFromDevice(){
  ...
  if (myPort.available() >= 1) { //originally 18 for 4 floats, but why?
...
//      encoders[0] = convertsCharStringToValue(inputString,0,4)-usigned_offset;
//      encoders[1] = convertsCharStringToValue(inputString,4,4)-usigned_offset;
//      encoders[2] = convertsCharStringToValue(inputString,8,4)-usigned_offset;
//
//      //get q's
//      q[0] = convertsCharStringToValue(inputString,0,2);
//      q[1] = convertsCharStringToValue(inputString,2,2);
//      q[2] = convertsCharStringToValue(inputString,4,2);
//      q[3] = convertsCharStringToValue(inputString,6,2);
//
...
/// default
      ...
      if(inputString != null){
        ...
        if(inputStringArr.length >= 8) { // q1,q2,q3,q4,x,y,z\r\n so we have 8 elements
          encAngles[0] = decodeFloat(inputStringArr[0]);
          encAngles[1] = decodeFloat(inputStringArr[1]);
          encAngles[2] = decodeFloat(inputStringArr[2]);
          ...
          println(q[0] + ", " + q[1] + ", " + q[2] + ", " + q[3] + ", " + encAngles[0] + ", " + encAngles[1] + ", " + encAngles[2]);
          //get  transfer time
          int dT = millis() - t1;
